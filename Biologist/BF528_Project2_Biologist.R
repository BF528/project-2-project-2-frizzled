...
#P0
P01 <- read.table("/projectnb/bf528/users/frizzled/project_2/Programmer/Rerun/P0_1_cufflinks/genes.fpkm_tracking", header=TRUE) %>% arrange(.,tracking_id)
colnames(P01)[10] <- "P0_1_FPKM"
P02 <- read.table("P02genes.fpkm_tracking", header=TRUE) %>% arrange(.,tracking_id)
colnames(P02)[10] <- "P0_2_FPKM"

#P4
P41 <- read.table("P41genes.fpkm_tracking", header=TRUE) %>% arrange(.,tracking_id)
colnames(P41)[10] <- "P4_1_FPKM"
P42 <- read.table("P42genes.fpkm_tracking", header=TRUE) %>% arrange(.,tracking_id)
colnames(P42)[10] <- "P4_2_FPKM"

#P7
P71 <- read.table("P71genes.fpkm_tracking", header=TRUE) %>% arrange(.,tracking_id)
colnames(P71)[10] <- "P7_1_FPKM"
P72 <- read.table("P72genes.fpkm_tracking", header=TRUE) %>% arrange(.,tracking_id)
colnames(P72)[10] <- "P7_2_FPKM"

#Ad
Ad1 <- read.table("Ad1genes.fpkm_tracking", header=TRUE) %>% arrange(.,tracking_id)
colnames(Ad1)[10] <- "Ad_1_FPKM"
Ad2 <- read.table("Ad2genes.fpkm_tracking", header=TRUE) %>% arrange(.,tracking_id)
...
gene_de <- read.table("/projectnb/bf528/users/frizzled/project_2/Programmer/rerun_cuffdiff/gene_exp.diff", sep="\t", header=TRUE) %>% filter(status=="OK")
...
pal <- rev(colorRampPalette(c("red3", "white", "blue2"))(100))  #color palette
...
png(filename="heatmap_bio.png", width=950, height=950, res=200)
pheatmap::pheatmap(f_ready, col=pal, scale="row", cluster_rows = TRUE, clustering_distance_rows = "euclidean", cluster_cols=as.hclust(cold), show_rownames = FALSE, main = "Gene Expression\n", fontsize = 13, legend=TRUE, angle_col = 90, fontsize_col = 10)
dev.off()